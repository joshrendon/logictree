...
from logictree.transforms.case_to_mux import case_to_mux_tree
...
def _two_way_case(left_expr, right_expr):
    ...
    i0 = CaseItem(match=LogicConst(0),
                  labels=[LogicConst(0)], 
                  body=BlockStatement(
                  statements=[LogicAssign("out", left_expr),]
                  ),
                  default=EMPTY_BRANCH)
    i1 = CaseItem(match=LogicConst(1),
                  labels=[LogicConst(1)],
                  body=BlockStatement([LogicAssign("out", right_expr),]
                  ),
                  default=EMPTY_BRANCH)
...
def test_case_lowers_to_if_then_mux_and_simplifies_to_aoi():
    ...
    lowered_if = case_to_if_tree(case_stmt)
    ...
    mux_tree = case_to_mux_tree(case_stmt)
    log.debug(f"case_to_mux_tree: {pretty_print(mux_tree)}")
    mux_expr = to_primitives_logic_tree(mux_tree)
    log.debug(f"to_primitives: {pretty_print(mux_expr)}")
    ...
    log.debug(f"simplify_logic_tree(mux_expr): {pretty_print(simplified)}")
...
def test_mux_simplifies_with_constant_arm_to_expected_form():
    ...
    mux_tree2 = case_to_mux_tree(case_stmt2)
    mux_expr2 = to_primitives_logic_tree(mux_tree2)
